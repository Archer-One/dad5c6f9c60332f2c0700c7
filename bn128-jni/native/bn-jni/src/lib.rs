...
#[no_mangle]
pub extern "system" fn Java_org_aion_tetryon_AltBn128Jni_HashToG1(
    env: JNIEnv,
    class: JClass,
    a_j: jbyteArray,
) -> jbyteArray {
    let a_byte = env.convert_byte_array(a_j).unwrap();
    let mut p1_byte:[u8; FP_SIZE * 2] = [0; FP_SIZE * 2];

    let ret = alt_bn128_HashToG1_internal(&a_byte,&mut p1_byte).unwrap();
    let output = env.byte_array_from_slice(&p1_byte).unwrap();

    output
}

fn alt_bn128_HashToG1_internal(a_byte: &[u8],p1_byte:&mut [u8]) -> Result<(), PairingErr> {
    let mut a = U256::from_slice(a_byte)?;
    a.add(&U256::zero(),&Fq::modulus());//将a mod上Fq的域值，防止随机值太大超出Fq域
    let mut x = Fq::from_u256(a).unwrap()-Fq::one();
    let mut z = Fq::zero();
    let mut y = Fq::zero();
    //let mut cont = 0;
    let mut success = false;
    while !success {//计算(x,y)，要是y不符合Fq的定义，则x+1，继续计算
        //cont += 1;
        x = x+Fq::one();
        z = x*x*x+bn::G1::b();
        match z.sqrt() {
    ...
    let g1_xy = {g1
        .x()
        .into_u256()
        .to_big_endian(&mut p1_byte[0..32])
        .unwrap();
            g1
        .y()
        .into_u256()
        .to_big_endian(&mut p1_byte[32..64])
        .unwrap();
    };
    //let (y, success) = safe_sqrt(&z);//重复hash
    // if let Some(g1) = AffineG1::new(x, y).unwrap(){
    //     g1.x()into_u256().to_big_endian(&mut p1_byte)[0..FP_SIZE]?;
    //     g1.y()into_u256().to_big_endian(&mut p1_byte)[FP_SIZE..FP_SIZE*2]?;
    // }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn my_HashToG1_2_test() {
        let rng = &mut ::rand::thread_rng();
        let mut a = U256::random(rng,&Fq::modulus());
        a.add(&U256::zero(),&Fq::modulus());//将a mod上Fq的域值，防止随机值太大超出Fq域
        let mut x = Fq::from_u256(a).unwrap()-Fq::one();
        let mut z = Fq::zero();
        let mut y = Fq::zero();
        ...
        println!("{}", hex::encode(&buffer[0..32]));
    }

    #[test]
    fn my_HashToG1_test() {
        let mut a = U256::from(1484949984845);//大数会出问题，1484949984845
        a.add(&U256::zero(),&Fq::modulus());//将a mod上Fq的域值，防止随机值太大超出Fq域
        let mut x = Fq::from_u256(a).unwrap()-Fq::one();
        let mut z = Fq::zero();
        let mut y = Fq::zero();
        let mut cont = 0;
        let mut success = false;
        while !success {//计算y，要是y不符合Fq的定义，则继续计算
            cont += 1;
            x = x+Fq::one();
            z = x*x*x+bn::G1::b();
            match z.sqrt() {
        ...
        println!("g1_x: {}", hex::encode(&buffer[0..64]));
        println!("{:?}", cont);
        //println!("{:?}", x);
        //println!("{:?}", y);
        //println!("{:?}", g1);
        ...
        //println!("{:?}", g1.x().into_u512().to_big_endian(&mut p1[0..FP_SIZE]));
        //println!("{:?}", g1.y().into_u512().to_big_endian(&mut p1[FP_SIZE..FP_SIZE * 2]));
    }
}
